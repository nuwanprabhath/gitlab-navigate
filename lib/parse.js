...
/**
 * GitLab's pipeline list. `scope=all` spans every ref rather than the default branch,
 * and `username` narrows to whoever triggered the run.
 */
function pipelineListUrl(base, extra) {
  if (!base) throw new ParseError('Set your GitLab repo URL first');

  const params = new URLSearchParams({ status: 'running', scope: 'all', ...extra });
  return `${base}/-/pipelines?${params.toString().replace(/\+/g, '%20')}`;
}
...
export function myPipelinesUrl(base, username) {
  if (!base) throw new ParseError('Set your GitLab repo URL first');
  if (!username) throw new ParseError('Set your GitLab username first');
  return pipelineListUrl(base, { username });
}
