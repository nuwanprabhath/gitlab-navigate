...
/**
 * The blank "new merge request" page, letting GitLab pick source/target itself.
 *
 * @param {string} base normalized repo root URL
 * @returns {string} the URL to open
 */
export function newMrUrl(base) {
 ...
 * @returns {string} the URL to open
 */
export function assignedMrUrl(base, username) {
  return mrListUrl(base, username, 'reviewer_username');
}

/**
 * Open MRs where the given user is the assignee, newest first.
 *
 * @param {string} base     normalized repo root URL
 * @param {string} username GitLab username, e.g. "nuwan-tern"
 * @returns {string} the URL to open
 */
export function assigneeMrUrl(base, username) {
