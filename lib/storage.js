...
/**
 * Move a pinned pipeline to a new position in the list.
 * @param {string} base - the pipeline's repo base URL
 * @param {string} id - the pipeline ID
 * @param {number} targetIndex - 0-based target position in the list
 * @returns {Promise<Array>} the reordered pinned list
 */
export async function reorderPinned(base, id, targetIndex) {
  const previous = await getPinned();
